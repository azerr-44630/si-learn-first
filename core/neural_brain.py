...
class NeuralBrain:
    ...
    def __init__(self):
        # Intent neyronlarının çəkiləri və açar sözlər
        self.neurons = {
    ...
    def evaluate(self, input_text):
        ...
        # Ən yüksək aktivasiya verən neyronu seçirik
        ...
        thought_chain = [
            f"🧠 [DÜŞÜNCƏ ZƏNCİRİ]: Daxil olan giriş tahlil edilir -> '{input_text}'",
            f"⚡ [NEYRON AKTİVASİYASI]: Ən yüksək reaksiya: {best_intent} (Səviyyə: {confidence*100:.1f}%)"
        ]

        if confidence < 0.5:
            thought_chain.append("⚠️ [QƏRAR]: Əminlik aşağıdır, ümumi daxili yaddaşa müraciət edilir.")
            decision = "UNKNOWN"
        else:
